...
class Cache {
	...
	private function __construct() {
		...
		add_action( 'decouplix_purge_paths_async', array( $this, 'purge_paths' ) );
		add_action( 'decouplix_purge_paths_cron', array( $this, 'purge_paths' ) );
	}
	...
	public function enqueue_purge( $paths ) {
		if ( function_exists( 'as_enqueue_async_action' ) ) {
			as_enqueue_async_action( 'decouplix_purge_paths_async', array( $paths ), 'decouplix' );
		} else {
			wp_schedule_single_event( time(), 'decouplix_purge_paths_cron', array( $paths ) );
		}
	}
	...
	public function purge_paths( $paths ) {
		...
		foreach ( $endpoints as $endpoint ) {
			...
			do_action( 'decouplix_cache_purged', $endpoint, $paths );
		}
	}
}
