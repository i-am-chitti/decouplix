...
class Queue {
	...
	private function __construct() {
		add_action( 'hc_webhook_triggered', array( $this, 'enqueue_webhook' ) );
		add_action( 'hc_deliver_webhook_async', array( $this, 'deliver_webhook' ) );
		add_action( 'hc_deliver_webhook_cron', array( $this, 'deliver_webhook' ) );
	}
	...
	public function enqueue_webhook( $payload ) {
		if ( function_exists( 'as_enqueue_async_action' ) ) {
			as_enqueue_async_action( 'hc_deliver_webhook_async', array( $payload ), 'headless-companion' );
		} else {
			wp_schedule_single_event( time(), 'hc_deliver_webhook_cron', array( $payload ) );
		}
	}
	...
	public function deliver_webhook( $payload ) {
		...
		do_action( 'hc_webhook_delivered', $response, $payload );
